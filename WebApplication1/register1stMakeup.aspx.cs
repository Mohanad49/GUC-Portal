...
namespace Admin
{
    public partial class register1stMakeup : System.Web.UI.Page
    {
        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    SqlCommand reg1MakeUp = new SqlCommand("[Procedures_StudentRegisterFirstMakeup]", conn);
                    reg1MakeUp.CommandType = CommandType.StoredProcedure;
                    reg1MakeUp.Parameters.Add(new SqlParameter("@StudentID", storedUserId));
                    reg1MakeUp.Parameters.Add(new SqlParameter("@CourseID", courseID));
                    reg1MakeUp.Parameters.Add(new SqlParameter("@studentCurr_sem", SemesterCode));
                    conn.Open();
                    int rowAffected = reg1MakeUp.ExecuteNonQuery();
                    if (rowAffected > 0)
                    {
                        formattedText = $@"
